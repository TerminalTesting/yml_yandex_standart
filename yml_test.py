...
class YMLTest(unittest.TestCase):
    ...
    def test_yml_0(self):
        ...
        try:
            ...
            print '-'*80
            print ('File: %s last error line: %s\n')%(self.YML_FILE_NAME, error_msg.last_error.line)
    ...
    def test_yml_1(self):
        ...
        try:
            ...
            print 'Error validating yml file'
            print ('File:%s\nLine:%s\nError message:%s')%(self.YML_FILE_NAME, error_msg.last_error.line, error_msg.last_error.message   )
            raise AssertionError
...
#             delivery_price={1:0.00, 2:0.00}
        ...
        for element in xml_tree:
            ...
            vendor_tag=element.find('vendor')
...
#                 print 'Error:NAME:', element.attrib['id'] ,' XML:',description_tag.text, ' DB:', item[0].name
            ...
            if int(float(price_tag.text)) == 0:
                ...
                if actions_goods: 
                    ...
                    if action_price_guid:
                        ...
                        good_price = session.query(City_price.price_type_guid).filter(City_price.city_id == item[2].id).filter(City_price.price_type_guid.in_(action_price_guid)).all() #guid акционной цены товара в нужном регионе
                        if good_price:
                            good_price = good_price[0][0]
            ...
            if (vendor_tag.text )!=(item[0].brand):
                stat+=1
                    ...
                    if DPD == True:
                        ...
                        if cost:
                            ...
                            if cost:
                                ...
                                print 'ID товара: ', element.attrib['id'], cost
                                print '-'*80
    ...
    def test_yml_3(self):
		...
		if link.code>400:
                    print ('%d.Error Stat:%d Link:%s')%(cnt, link.code, element[1].text )
	            stat +=1
		else:
